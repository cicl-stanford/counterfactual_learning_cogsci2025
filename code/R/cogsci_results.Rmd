...
data_dir_s1 = '../../data/s1_quicksand/pilot5/'
data_dir_s2 = '../../data/s2_quicksand/fullstudy/'
figures_dir = '../../figures/cogsci2025/'

# set brms to refit models every time. comment out when we think things converge
...
# only do if user if me (justyang)
...
session_df_s2 = read_csv(file.path(data_dir_s2, 'session_data_r.csv'))
world_df_s2 = read_csv(file.path(data_dir_s2, 'world_data_r.csv'))
quicksand_df_s2 = read_csv(file.path(data_dir_s2, 'trial_data_r.csv'))
...
quicksand_df_s2 <- quicksand_df_s2 %>% 
  mutate(condition = factor(condition, levels = c("observation", "hypothetical", "counterfactual")))  # Even if 
```
...
fit.s1_manipulation_check = quicksand_df_s1 %>% 
  ...
  mutate(counterfactual = as.numeric(trial_type == 'quicksand-simulate')) %>% 
  brm(
...
```{r}
fit.s1_flat_priors = quicksand_df_s1 %>% 
  left_join(world_df_s1 %>% select(world_id, world_spec_id), by = "world_id") %>%
  rename(environment = world_spec_id) %>%
  filter(condition %in% c('hypothetical', 'counterfactual')) %>%
  mutate(condition = factor(condition, levels = c("hypothetical", "counterfactual"))) %>%
  brm(
    formula = simulation_hazard_count_flat_priors ~ 1 + condition + (1 + condition | game_id) + (1 | environment),
    family = poisson(),
    seed = 1,
    cores = parallel::detectCores(), # refit model
    file_refit = "always",
    file = 'cache/fit.s1_flat_priors'
  )

# summary(fit.s1_flat_priors)

fit.s1_flat_priors %>% 
  emmeans(specs=pairwise~condition, type = "response")
...
ggsave(file.path(figures_dir, "temp.pdf"), width = 7, height = 3, useDingbats = FALSE)
...
# Pilot Study: Unsafe Tiles in Path
fig_s1_hyp1 <- quicksand_df_s1 %>%
  filter(trial_type == "quicksand-planner") %>%
  ggplot(aes(x = days_binned, 
             y = hazard_count, 
             color = condition, 
             group = condition)) +
  stat_summary(fun = mean, 
  ...
  scale_y_continuous(breaks = seq(4.0, 5.4, 0.2), 
                     expand = expansion(mult = 0.01)) +
  coord_cartesian(ylim = c(4.0, 5.4)) +
  labs(title = "Study 1",
       x = "Trial order (binned)",
       y = "Unsafe tiles",
       color = "Condition") + 
  guides(color = guide_legend(nrow = 1))

# Full Study: Unsafe Tiles in Path
fig_s2_hyp1 <- quicksand_df_s2 %>%
  filter(trial_type == "navigation") %>%
  ggplot(aes(x = days_binned, 
             y = hazard_count, 
             color = condition, 
  ...
  scale_color_manual(values = condition_palette) +
  scale_x_discrete(labels = day_split_labels_new) +
  scale_y_continuous(breaks = seq(3.8, 4.5, 0.1), 
                     expand = expansion(mult = 0.01)) +
  coord_cartesian(ylim = c(3.8, 4.5)) +
  labs(title = "Study 2",
       y = " ",
       x = "Trial order (binned)") +
  guides(color = "none")
...
```{r}
# Pilot Study: Individual Unsafe Tiles in Path
fig_s1_hyp1 <- quicksand_df_s1 %>%
  filter(trial_type == "quicksand-planner") %>%
  ggplot(aes(x = day, 
             y = hazard_count, 
             color = condition, 
             group = game_id)) +  # Individual trajectories
  geom_line(alpha = 0.3, size = 0.5) +  # Many lines, so lower alpha
  scale_color_manual(values = condition_palette,
                     labels = condition_legend_labels) +
  # scale_y_continuous(breaks = seq(4.0, 5.4, 0.2), 
  #                    expand = expansion(mult = 0.01)) +
...
# Full Study: Individual Unsafe Tiles in Path
fig_s2_hyp1 <- quicksand_df_s2 %>%
  filter(trial_type == "navigation") %>%
  ggplot(aes(x = day, 
             y = hazard_count, 
  ...
  labs(title = "Study 2",
       y = " ",
       x = "Day") +
  guides(color = "none")  # Remove legend since shared in first plot
...
#   scale_color_manual(values = condition_palette,
#                      labels = condition_legend_labels) + 
#   scale_x_discrete(labels = day_split_labels) +
#   scale_y_continuous(breaks = seq(3.8, 5.4, 0.2), 
#                      expand = expansion(mult = 0.01)) +
#   coord_cartesian(ylim = c(3.8, 5.4)) +
#   labs(title = NULL,
#        x = "Trial order (binned)",
#        y = "Unsafe tiles",
#        color = "Condition") + 
...
#         legend.margin = margin(-10, 0, 0, 0))
# 
# ggsave(file.path(figures_dir, "hypothesis1_yshared.pdf"), 
#        units = "cm", 
#        useDingbats = FALSE)
# 
# fig_combined
