...
```{r}
quicksand_df_s1 %>%
  filter(world_id == '2139-5c7ee136-9214-458c-85c0-388bc9cb44da')

model_df_s1 %>% 
  left_join(quicksand_df_s1 %>% select(trial_id, world_id, day), by = "trial_id") %>%
  filter(world_id == '2139-5c7ee136-9214-458c-85c0-388bc9cb44da')
```
...
fig_s1_hyp1 <- combined_s1_all %>%
  ...
  scale_color_manual(values = condition_palette, labels = condition_legend_labels) + 
  scale_y_continuous(breaks = seq(2.8, 6.4, 0.2), expand = expansion(mult = 0.01)) +
  coord_cartesian(ylim = c(2.8, 6.4)) +
...
fig_s2_hyp1 <- combined_s2_all %>%
  ...
  geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs"), se = TRUE) +
  scale_color_manual(values = condition_palette, labels = condition_legend_labels) +
  scale_y_continuous(breaks = seq(2.0, 6.0, 0.5), expand = expansion(mult = 0.01)) +
  coord_cartesian(ylim = c(2.0, 6.0)) +
