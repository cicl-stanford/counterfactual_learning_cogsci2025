...
# Navigation trial hazard count
# functions to read json columns
def decode_json(json_str):
    try:
        parsed = json.loads(json_str)
        
        if isinstance(parsed, dict):
            # Handle dictionaries with string tuple keys
            return {ast.literal_eval(k) if isinstance(k, str) and k.startswith("(") else k: v \
                    for k, v in parsed.items()}
        elif isinstance(parsed, list):
            # Handle lists of dictionaries or other structures
            return [decode_json(json.dumps(item)) if isinstance(item, dict) else item for item in parsed]
        else:
            # For all other structures, return as-is
            return parsed
...
# Convert json strings to the right data structure
...
# add connected navigation trial trial_id to simulation trials in quicksand_df
...
# #### Compute baselines for manipulation check section
...
# ===== Quicksand and Custom Libraries =====
